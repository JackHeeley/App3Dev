//
// exception.hpp : implements exception message support
//
...
///<summary>use the predefined ANSI/ISO C99 C preprocessor macros to inject locus & context details when constructing a fox::exception</summary>
#define fox_exception(_text) fox::exception(__FILE__, __LINE__, __func__, (_text))

namespace fox
{
   ///<summary> a "preferred" exception type with added locus/context details.</summary>
   ///<remarks> the macro 'fox_exception(_text)' will construct one of these for you.</remarks>
   class BASICUNIVERSALCPPSUPPORT_API exception : public std::exception
   {
   ...
   public:
      ///<summary> construct a fox::exception (an exception type with added locus and context details).</summary>
      ///<remarks> macro 'fox_exception(_text)' will construct one in line.</remarks>
      ...
      exception(const char* a_path, int a_line, const char* a_func, const char* a_what) :
